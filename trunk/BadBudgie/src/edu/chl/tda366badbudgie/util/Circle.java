...
public class Circle extends Polygon {
	...
	private static List<Vector> circleVertices(Vector position, double radius, 
			int resolution){
		...
		for(int i = 0; i < resolution; i++){
			double x = 
				position.getX() + radius * Math.cos(i*((Math.PI*2)/resolution));
			double y =
				position.getY() + radius * Math.sin(i*((Math.PI*2)/resolution));
